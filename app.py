...
7. Tournaments (150): across 33 countries, Spain hosts the most (37)
8. Top Tournaments: Premier Padel Riyadh — 15.2M viewers, €3.2M revenue, 99% occupancy
9. Sponsorship: Estrella Damm €8.5M/year, 185% ROI
10. Player Contracts: Arturo Coello €450k with Bullpadel, avg contract €161,850
11. Social Media: Arturo Coello 2.8M Instagram followers, avg sponsorship value €533k/year
12. Shoes (78): avg €83.5, Asics leads with 26 products
13. Seasons: 6 seasons 2023–2026, best year 2025 with 40 tournaments
14. FIP Titles: J. Luis Gonzalez & M. Cassetta lead with 13 titles each

Answer concisely. Cite specific numbers when available. Keep answers under 150 words unless a breakdown is requested."""
...
def render_chatbot():
    ...
    st.markdown("Ask anything about players, tournaments, rackets, or sponsorships.")
    ...
    for i, s in enumerate(SUGGESTIONS):
        if cols[i % 3].button(s, key=f"chip_{i}", use_container_width=True):
    ...
    # ── Input ──────────────────────────────────────────────────
    prompt = st.chat_input("Ask about players, rankings, tournaments...")
...
with st.sidebar:
    ...
    st.markdown('<div class="sidebar-section">🏟️ Tournaments</div>', unsafe_allow_html=True)
...
if st.session_state.active_group == "gen":
    ...
    active = player_sel
elif st.session_state.active_group == "nlp":     # NEW
    active = nlp_sel                              # NEW
else:
    active = tour_sel
...
st.markdown(
    '<div class="sub-header">Machine Learning Models · Players & Tournaments · Professional Padel Circuit</div>',
    unsafe_allow_html=True
)
st.markdown("---")
...
<div style='text-align:center;color:#8A9BB0;font-size:0.85rem;letter-spacing:0.06em;'>
    🎾 PADEL ANALYTICS ML DASHBOARD &nbsp;·&nbsp; Players & Tournaments &nbsp;·&nbsp; Built with Streamlit & Scikit-learn
</div>
""", unsafe_allow_html=True)